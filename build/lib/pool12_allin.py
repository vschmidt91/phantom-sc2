...
def get_intersections(p0: Point2, r0: float, p1: Point2, r1: float) -> Iterable[Point2]:
    """Return the intersection points of two circles at points p0, p1 with radii r0, r1"""
    p01 = p1 - p0
...
class Pool12AllIn(BotAI):
    ...
    async def on_step(self, iteration: int) -> None:
        ...
        if any(self.enemy_structures.flying) and not any(
        ...
        self.abilities: Counter[AbilityId] = Counter(
            o.ability.exact_id for u in self.all_own_units for o in u.orders
        )
    ...
    def build_spire(self) -> bool:
        ...
        if self.spire:
            ...
            if (
                self.abilities[AbilityId.ZERGBUILD_SPIRE] < 1
                and self.can_afford(UnitTypeId.SPIRE)
                and self.drone
            ):
                self.drone(AbilityId.ZERGBUILD_SPIRE, self.spire_position)
        elif (
            self.abilities[AbilityId.UPGRADETOLAIR_LAIR] < 1
    ...
    def build_order(self) -> bool:
        if not self.pool and self.abilities[AbilityId.ZERGBUILD_SPAWNINGPOOL] < 1:
            ...
            self.research_speed
            and self.gas_buildings.amount < 1
            and self.abilities[AbilityId.ZERGBUILD_EXTRACTOR] < 1
        ):
            if self.drone:
            ...
            self.supply_cap == 14 and self.abilities[AbilityId.LARVATRAIN_OVERLORD] < 1
        ):
    ...
    def macro(self, build_army: bool) -> None:
        ...
        minerals_for_lings = (
            50 * 60 * larva_per_second
        )  # maximum we can possibly spend on lings
        mineral_starved = (
            self.minerals < 150
            and self.state.score.collection_rate_minerals < 1.2 * minerals_for_lings
        )  # aim for a 20% surplus
        drone_max = sum(hatch.ideal_harvesters for hatch in self.townhalls)
        queen_missing = self.townhalls.amount - (
            len(self.inject_queens) + self.abilities[AbilityId.TRAINQUEEN_QUEEN]
        )
        if self.larva and 1 <= self.supply_left:
            ...
            self.supply_left < self.larva.amount
            and self.abilities[AbilityId.LARVATRAIN_OVERLORD] < 1
        ):
            self.train(UnitTypeId.OVERLORD)
    ...
    def micro_structure(self, unit: Unit) -> None:
        if not unit.is_ready and unit.health_percentage < 0.1:
            ...
            if (
                unit.is_ready
                and unit.assigned_harvesters + 1 < self.gas_harvester_target
            ):
                self.transfer_to_gas.extend(
                    unit
                    for _ in range(
                        unit.assigned_harvesters + 1, self.gas_harvester_target
                    )
                )
            elif self.gas_harvester_target < unit.assigned_harvesters:
                self.transfer_from_gas.extend(
                    unit
                    for _ in range(self.gas_harvester_target, unit.assigned_harvesters)
                )
            ...
            if unit.is_ready:
                ...
                if 0 < unit.surplus_harvesters:
                    self.transfer_from.extend(
                        unit for _ in range(0, unit.surplus_harvesters)
                    )
                elif unit.surplus_harvesters < 0:
                    self.transfer_to.extend(
                        unit for _ in range(unit.surplus_harvesters, 0)
                    )
            else:
    ...
    def micro_worker(self, unit: Unit) -> None:
        if unit.is_idle:
            if any(self.mineral_field) and (
                not self.pool_drone or self.pool_drone.tag != unit.tag
            ):
                townhall = self.townhalls.closest_to(unit)
                patch = self.mineral_field.closest_to(townhall)
                unit.gather(patch)
        elif (
            any(self.transfer_from)
            and any(self.transfer_to)
            and unit.order_target == self.transfer_from[0].tag
        ):
        ...
        if self.speedmining_enabled and len(unit.orders) == 1:
            ...
            if unit.is_returning:
                ...
                move_target = target.position.towards(
                    unit.position, target.radius + unit.radius
                )
            ...
            if (
                target
                and 2 * unit.radius
                < unit.distance_to(move_target)
                < SPEEDMINING_DISTANCE
            ):
    ...
    def inject_larvae(self):
        ...
        for hatch, queen in zip(hatches, self.inject_queens):
            if 25 <= queen.energy and hatch.is_ready:
                ...
                target = hatch.position.towards(
                    self.game_info.map_center, hatch.radius + queen.radius
                )
    ...
    async def get_pool_position(self) -> Point2:
        ...
        for i in range(100):
            position = self.start_location.towards_with_random_angle(
    ...
    def split_workers(self) -> None:
        ...
        minerals = self.expansion_locations_dict[
            self.start_location
        ].mineral_field.sorted_by_distance_to(self.start_location)
