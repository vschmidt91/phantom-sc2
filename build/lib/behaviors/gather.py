    ...
    def on_step(self) -> None:
        ...
        if self.ai.debug:
            ...
            if self.gather_target is not None:
                ...
                self.ai.client.debug_line_out(position_from, position_to, color=(0, 255, 0))
        ...
        #     self.unit(AbilityId.SMART, self.return_target.unit, True)
            ...
            if self.state.is_returning:
                ...
                move_target = townhall.position.towards(
                    self.state, townhall.radius + self.state.radius
                )
                ...
                if self.state.order_target != target.tag:
                    ...
                    if not move_target:
                        move_target = target.position.towards(
                            self.state, target.radius + self.state.radius
                        )
