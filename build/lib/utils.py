...
) -> Iterable[Union[UnitTypeId, UpgradeId]]:
...
def flood_fill_incremental_bool(is_border: np.ndarray, origins: Set[Point2]):
    ...
    front = {(int(x), int(y)) for x, y in origins}
    for d in itertools.count():
        ...
        front = {p for p in front if d < distance[p] and not is_border[p]}
    ...
    while front:
        ...
        for point in front:
            ...
            for offset in FLOOD_FILL_OFFSETS:
                ...
                neighbour_distance = (
                    point_distance + offset_norm * weight[neighbour.rounded]
                )
