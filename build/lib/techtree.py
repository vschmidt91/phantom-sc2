...
class TechTreeAbility:
    ...
    cooldown: int
    target: Union[
        TechTreeAbilityTarget, TechTreeAbilityTargetResearch, TechTreeAbilityTargetUnit
    ]
...
class TechTreeUnit:
    ...
    def can_attack_ground(self) -> bool:
        if self.id in {UnitTypeId.BATTLECRUISER, UnitTypeId.ORACLE}:
            return True
        return any(
            weapon.target_type & TechTreeWeaponType.GROUND for weapon in self.weapons
        )
    ...
    def can_attack_air(self) -> bool:
        if self.id in {UnitTypeId.BATTLECRUISER}:
            return True
        return any(
            weapon.target_type & TechTreeWeaponType.AIR for weapon in self.weapons
        )
    ...
    def __init__(self, path: str) -> None:
        ...
        for item in data["Ability"]:
            ...
            if isinstance(item["target"], dict):
                ...
                if key == "Research":
                    item["target"] = TechTreeAbilityTargetResearch(
                        UpgradeId(value["upgrade"])
                    )
                else:
                ...
                item["target"] = TechTreeAbilityTarget[
                    camel_to_upper_case(item["target"])
                ]
