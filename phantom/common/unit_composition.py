...
class UnitComposition(Mapping[UnitTypeId, float]):
    ...
    def __add__(self, other: Mapping[UnitTypeId, float]) -> "UnitComposition":
        return UnitComposition({k: self.get(k, 0.0) + other.get(k, 0.0) for k in set(self) | set(other)})

    def __sub__(self, other: Mapping[UnitTypeId, float]) -> "UnitComposition":
        return UnitComposition({k: max(0.0, self.get(k, 0.0) - other.get(k, 0.0)) for k in set(self) | set(other)})
