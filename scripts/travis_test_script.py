...
"""
This script is made as a wrapper for sc2 bots to set a timeout to the bots
(in case they can't find the last enemy structure or the game is ending in a draw)
Ideally this script should be done with a bot that terminates on its own after certain things have been achieved
e.g. testing if the bot can expand at all, and then terminates after it has successfully expanded.
...
if len(sys.argv) > 1:
    ...
    for i in range(retries):
        ...
        try:
            # Stop the current bot if the timeout was reached
            # the bot needs to finish a game within 3 minutes real time
    ...
    if process.returncode is None or process.returncode != 0:
        print(
            f"Exiting with exit code 5,"
            f"error: Attempted to launch script {sys.argv[1]} timed out after {time_taken} seconds."
            f"Retries completed: {i}"
        )
        exit(5)

    # process.returncode will always return 0
    # if the game was run successfully
    # or if there was a python error (in this case it returns as defeat)
