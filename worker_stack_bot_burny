            ...
            for worker in self.workers:
                ...
                if len(worker.orders) == 1:
                    if not worker.is_carrying_minerals:
                        ...
                        elif worker.distance_to(mineral) > mineral.radius + worker.radius + self.townhall_distance_threshold:
                            pos: Point2 = mineral.position
                            worker.move(pos.towards(worker, mineral.radius * self.townhall_distance_factor))
                            worker.smart(mineral, queue=True)
                        else:
                            worker.smart(mineral)
