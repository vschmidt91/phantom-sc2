    ...
    def add(self, unit: Unit, bonus_distance: float) -> None:
        ...
        if unit.is_flying:
            if self.ai.techtree.units[unit.type_id].can_attack_ground:
                self.air_vs_ground = self.ai.map_analyzer.add_cost(unit.position, range_vs_ground, self.air_vs_ground,
                                                                   weight)
            ...
            if self.ai.techtree.units[unit.type_id].can_attack_ground:
                self.ground_vs_ground = self.ai.map_analyzer.add_cost(unit.position, range_vs_ground,
                                                                      self.ground_vs_ground, weight)
            if self.ai.techtree.units[unit.type_id].can_attack_air:
                self.ground_vs_air = self.ai.map_analyzer.add_cost(unit.position, range_vs_air, self.ground_vs_air,
                                                                   weight)
