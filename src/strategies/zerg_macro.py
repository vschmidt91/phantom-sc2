...
class ZergMacro(ZergStrategy):
    ...
    def composition(self) -> Dict[UnitTypeId, int]:
        ...
        if tech_up:
            ...
            if self.ai.count(UnitTypeId.LAIR, include_planned=False, include_pending=False):
        ...
        # if self.ai.count(UnitTypeId.ROACHWARREN, include_pending=False, include_planned=False):
    ...
    def filter_upgrade(self, upgrade) -> bool:
        if upgrade == UpgradeId.ZERGLINGMOVEMENTSPEED:
            return True
        # elif self.ai.count(UnitTypeId.LAIR, include_planned=False, include_pending=False) < 1:
        #     return False
