...
from ..constants import GAS_BY_RACE, ITEM_TRAINED_FROM_WITH_EQUIVALENTS, MACRO_INFO, REQUIREMENTS, WITH_TECH_EQUIVALENTS
...
class MacroModule(AIModule):
    ...
    async def on_step(self) -> None:
        ...
        for i, plan in enumerate(plans):
            ...
            if any(self.ai.get_missing_requirements(plan.item)):
                ...
                if 0 < cost.vespene:
                    eta = max(
                        eta,
                        60 * (reserve.vespene - self.ai.vespene) / max(1, self.ai.resource_manager.income.vespene),
                    )
                if 0 < cost.larva:
                    eta = max(
                        eta,
                        60 * (reserve.larva - self.ai.larva.amount) / max(1, self.ai.resource_manager.income.larva),
                    )
                if 0 < cost.food:
        ...
        future_spending += sum(
            (self.ai.get_cost(plan.item) for plan in self.ai.macro.unassigned_plans),
            cost_zero,
        )
        future_spending += sum(
            (
...
class MacroBehavior(AIUnit):
    ...
    def macro_ability(self) -> Optional[AbilityId]:
        if self.plan and (element := MACRO_INFO.get(self.state.type_id)) and (ability := element.get(self.plan.item)):
