...
class MacroModule(AIModule):
    ...
    async def on_step(self) -> None:
        ...
        for i, plan in enumerate(plans):
            ...
            if (
                2 if self.ai.extractor_trick_enabled else 1
            ) <= i and plan.priority == math.inf:
                break
            ...
            if any(self.ai.get_missing_requirements(plan.item)):
                ...
                if 0 < cost.minerals:
                    eta = max(
                        eta,
                        60
                        * (reserve.minerals - self.ai.minerals)
                ...
                if 0 < cost.vespene:
                    eta = max(
                        eta,
                        60
                        * (reserve.vespene - self.ai.vespene)
                ...
                if 0 < cost.larva:
                    eta = max(
                        eta,
                        60
                        * (reserve.larva - self.ai.larva.amount)
        ...
        if 0 < future_spending.minerals:
            future_timeframe = max(
                future_timeframe,
                future_spending.minerals
                / max(1, self.ai.resource_manager.income.minerals),
            )
        if 0 < future_spending.vespene:
            future_timeframe = max(
                future_timeframe,
                future_spending.vespene
                / max(1, self.ai.resource_manager.income.vespene),
            )
    ...
    def search_trainer(
        self, trainers: Iterable[MacroBehavior], item: MacroId
