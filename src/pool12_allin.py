    ...
    async def on_step(self, iteration: int):
        ...
        for unit in self.units:
            if unit.type_id is UnitTypeId.EGG:
                ...
                queen_count += 1
                if inject_hatch and 25 <= unit.energy and not unit.is_using_ability(AbilityId.EFFECT_INJECTLARVA):
                    unit(AbilityId.EFFECT_INJECTLARVA, inject_hatch)
        
        if self.state.game_loop % 2 == 1:
            pass
        elif not pool and not pool_pending:
            target = self.townhalls.first.position.towards(self.game_info.map_center, -10)
            if drone:
        ...
        elif idle_hatch and queen_count + queen_morphing_count < self.townhalls.ready.amount:
            ...
            self.train(UnitTypeId.OVERLORD)
        elif drone_morphing_count < 1 and drone_count + drone_morphing_count < drone_target:
            self.train(UnitTypeId.DRONE)
        elif self.can_afford(UnitTypeId.HATCHERY) and drone_target <= drone_count + drone_morphing_count and hatch_pending_count < 1:
